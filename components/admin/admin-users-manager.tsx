...
import {
  faCircleCheck, faEye, faEyeSlash, faFloppyDisk, faKey, faPlus,
...
export function AdminUsersManager({currentAdminId}:{currentAdminId:string}){
  const [rows,setRows]=useState<AdminRow[]>([]),[loading,setLoading]=useState(true),[busy,setBusy]=useState(false),[editorOpen,setEditorOpen]=useState(false),[deleteTarget,setDeleteTarget]=useState<AdminRow|null>(null),[showPassword,setShowPassword]=useState(false),[form,setForm]=useState<FormState>(emptyForm);
  ...
  function closeEditor(){if(busy)return;setEditorOpen(false);setShowPassword(false);setForm(emptyForm)}
  async function submit(event:FormEvent){
    ...
    try{const response=await fetch("/api/admin/admin-users",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:form.username,displayName:form.displayName,password:form.password,role:form.role})}),body=await readJson<{error?:string}>(response);if(!response.ok)throw new Error(body.error||"เพิ่มผู้ดูแลไม่สำเร็จ");toast.success("เพิ่มผู้ดูแลระบบเรียบร้อยแล้ว");setEditorOpen(false);setShowPassword(false);setForm(emptyForm);await load()}
    catch(error){toast.error(error instanceof Error?error.message:"เพิ่มผู้ดูแลไม่สำเร็จ")}finally{setBusy(false)}
  }
  ...
  return <>
    ...
    {editorOpen&&<div className="modal-backdrop" onMouseDown={event=>event.target===event.currentTarget&&closeEditor()}><section className="modal admin-user-modal" role="dialog" aria-modal="true" aria-labelledby="add-admin-title" onMouseDown={event=>event.stopPropagation()}>
      <header className="admin-user-modal-header"><div><span><FontAwesomeIcon icon={faUserShield}/></span><div><small>บัญชีผู้ดูแลใหม่</small><h2 id="add-admin-title">เพิ่มผู้ดูแลระบบ</h2></div></div><button className="icon-button" type="button" onClick={closeEditor} aria-label="ปิด"><FontAwesomeIcon icon={faXmark}/></button></header>
      <form onSubmit={submit} className="admin-user-form">
        ...
        <label className="field full"><span>ระดับสิทธิ์ <b>*</b></span><select value={form.role} onChange={event=>setForm({...form,role:event.target.value as FormState["role"]})}><option value="admin">ผู้ดูแลทั่วไป</option><option value="superadmin">ผู้ดูแลระดับสูง</option></select><small>ผู้ดูแลระดับสูงสามารถเพิ่มและลบบัญชีผู้ดูแลอื่นได้</small></label>
        <label className="field"><span>รหัสผ่าน <b>*</b></span><div className="password-field"><input required minLength={8} maxLength={128} type={showPassword?"text":"password"} autoComplete="new-password" value={form.password} onChange={event=>setForm({...form,password:event.target.value})} placeholder="อย่างน้อย 8 ตัวอักษร"/><button type="button" onClick={()=>setShowPassword(value=>!value)} aria-label={showPassword?"ซ่อนรหัสผ่าน":"แสดงรหัสผ่าน"}><FontAwesomeIcon icon={showPassword?faEyeSlash:faEye}/></button></div></label>
        <label className="field"><span>ยืนยันรหัสผ่าน <b>*</b></span><input required minLength={8} maxLength={128} type={showPassword?"text":"password"} autoComplete="new-password" value={form.confirmPassword} onChange={event=>setForm({...form,confirmPassword:event.target.value})} placeholder="กรอกรหัสผ่านอีกครั้ง"/></label>
        <div className="modal-actions full"><button className="button secondary" type="button" disabled={busy} onClick={closeEditor}>ยกเลิก</button><button className="button primary" disabled={busy}><FontAwesomeIcon icon={busy?faSpinner:faFloppyDisk} spin={busy}/>{busy?"กำลังบันทึก...":"บันทึกผู้ดูแล"}</button></div>
      </form>
    </section></div>}
